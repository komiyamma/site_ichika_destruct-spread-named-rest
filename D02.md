...
* `[` と `]` の形を見てください。これは **配列の形そのもの** です。<br>
...
要素が足りないと `undefined` になってしまうので、<br>
...
関数から「複数の値」を返したいとき、配列にして返すことがあります。<br>
...
```mermaid
flowchart LR
    Func[関数: makePair] -- "returns [10, 20]" --> Arr["戻り値: [10, 20]"]
    Arr --- Destruct[分割代入]
    
    Destruct -- "0番目" --> VarA[変数: a = 10]
    Destruct -- "1番目" --> VarB[変数: b = 20]

    style Func fill:#4a90d9,stroke:#333,color:#fff
    style Arr fill:#e6a23c,stroke:#333,color:#fff
```

> 💡 **React を学ぶ予定の方へ**<br>
> React の `useState` という機能は、まさにこの仕組みを使っています！<br>
> `const [count, setCount] = useState(0)` という書き方は、<br>
