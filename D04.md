...
> | 用語 | 英語 | 使う場面 | 例 |
> |------|------|----------|-----|
> | 残余要素 | Rest Element | 配列の分割代入 | `[a, ...rest] = arr` |
> | 残余引数 | Rest Parameter | 関数の引数 | `function fn(...args)` |
> | 残余構文 | Rest Syntax | 上記の総称 | ― |
...
💬 `"..."` の後ろにある配列の中身を「ばらして」別の配列の中に入れています。<br>
...
💬 `...others` は「残り全部」をまとめてくれる便利なバケツ。<br>
> 🧠 覚え方：「rest は“rest（残り）”！」
...
flowchart TD
    ...
    style Box1 fill:#e6a23c,stroke:#333,color:#fff
    style Box2 fill:#17a2b8,stroke:#333,color:#fff
```
...
💬 addressの中にあるキーを、さらに `{}` の中で展開できます。<br>
