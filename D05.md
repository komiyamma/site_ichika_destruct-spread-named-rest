...
flowchart LR
    ...
    style Args fill:#ccf,stroke:#333
```
...
💡 但し、これは「空オブジェクト」すら「引数で渡ってこない」ことまでもを仕様として想定した場合限定です。
　 普通は「引数を渡すことを縛る」場合がほとんどですので、この { } はあまり使いません。
...
> ?? `...numbers` は「残余引数（rest parameter）」で、必ず配列になります。
> ?? `arguments` は古い書き方で配列ではないため、普段は残余引数を使うのが安全です。
...
#### 💬「引数って、まるで荷物の山みたいでしょ？📦📦<br>　 　そのまま受け取るとぐっちゃぐちゃになるけど、<br>　 　`{ name, age }` で仕分けすればスッキリ！<br>　 　余った荷物は `...rest` の箱にポンっと入れとけばOKよ💨<br>　 　あたし？ もちろん仕分け最速のダンボ姉さんよ🧤✨」
