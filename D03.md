...
これでも動きますが、似たような行がずらっと並んでしまいます。<br>
...
左の `{ name, age, country }` は “抜き取りたい引き出し名” を表しています。<br>
...
flowchart LR
    ...
    style Source fill:#d63384,stroke:#333,color:#fff
```
...
flowchart TD
    ...
    style UserObj fill:#d63384,stroke:#333,color:#fff
    style Var fill:#e6a23c,stroke:#333,color:#fff
```

📦「箱の中の name を開けて userName に入れる」ということ！

<img src="./images/D03_label_change.png" width="500" alt="Rename label illustration" />
...
オブジェクトの中に、さらにオブジェクトが入っている状態のことです。
...
> 💡 `address: { city, zip }` と書くことで、addressの中をさらに展開しています。

```mermaid
flowchart TD
    User[User Object]
    
    subgraph Address [address オブジェクト]
    City[city: Tokyo]
    Zip[zip: 100-0001]
    end
    
    User -- name --> NameVar[name 変数]
    User -- address --> Address
    Address -- city --> CityVar[city 変数]
    Address -- zip --> ZipVar[zip 変数]
    
    style User fill:#d63384,stroke:#333,color:#fff
    style Address fill:#4a90d9,stroke:#333,color:#fff
```
...
console.log(city);    // Tokyo
console.log(address); // ❌ ReferenceError! address は変数として作られない
```

> 🧠 `address:` の部分は「ここを掘る」という指示であり、`address` という変数は作られません。
> `address` 自体も変数として欲しい場合は、別途取り出す必要があります。
...
### 🚨 注意：デフォルト値が効かないとき（`null` の罠）

「値がないとき」にデフォルト値が使われますが、<br>
JavaScript では **`undefined` のときだけ** が「値がない」とみなされます。

**`null` は「空っぽという値がある」扱い** になるので、デフォルト値は発動しません！

```javascript
const user = { name: null };

// name は "ゲスト" にならず、null のまま！
const { name = "ゲスト" } = user;
...
flowchart TD
    ...
    Input -- Yes --> Default[デフォルト値を使う]
    Input -- No --> Direct[そのままの値を使う]
    
    subgraph Examples
    Case1[undefined] -.-> Result1[デフォルト発動！]
    Case2[null] -.-> Result2[null のまま…]
    end
    
    style Case1 fill:#ccffcc,stroke:#333
    style Result1 fill:#ccffcc,stroke:#333
    
    style Case2 fill:#ffcccc,stroke:#333
    style Result2 fill:#ffcccc,stroke:#333
```

> ⚠️ API からのデータで `null` が入っている場合にハマりやすいポイントです。<br>
> `null` も弾きたい場合は `const name = user.name || "ゲスト"` のような書き方が必要なこともあります。
